...
function Pane({
  ...
  return (
    <div
      ...
      style={{ background: q.bg, border: `1px solid ${q.color}33`, minHeight: 168 }}
    >
...
export default function ResultsPage() {
  ...
  if (!data.available) {
    return (
      <div className="pt-10">
        <button className="text-sub text-sm inline-flex items-center gap-1.5" onClick={() => router.push("/home")}>
        ...
        <div className="rounded-xl border border-line bg-panel p-5 mt-4 text-sub">
          {data.reason === "period_open"
            ? "아직 진행 기간이에요. 기간이 끝나면 여기서 결과를 볼 수 있어요."
            : `아직 응답이 충분히 모이지 않았어요 (${data.evaluatorCount}/${data.minEvaluators}명). 익명성 보호를 위해 최소 인원이 모여야 결과를 볼 수 있어요.`}
        </div>
      </div>
    );
  }
    ...
    <div className="pt-6">
      ...
      <div className="flex items-end justify-between flex-wrap gap-3 mt-3">
        <div>
          ...
          <p className="text-sub text-sm mt-1.5">
            {evaluatorCount > 0 ? `동료 ${evaluatorCount}명이 내 창을 채워 주었어요.` : "아직 내 창을 채워 준 동료가 없어요."}{" "}
            숫자는 그 형용사를 고른 동료 수예요.
          </p>
        </div>
      </div>
      ...
      {evaluatorCount > 0 && (
        <div className="flex gap-3 mt-4">
          <MetricCard
            ...
            desc="내가 아는 나 중에서 동료에게도 보이는 비율. 자기개방의 정도예요 — 높을수록 숨겨진 창이 작아요. (숨겨진 → 열린 창으로 넓혀가는 방향)"
          />
        </div>
      )}
      ...
      <div className="mt-5">
        <div className="grid gap-2.5 items-center" style={{ gridTemplateColumns: "22px 1fr 1fr" }}>
          <div />
          <AxisLabel text="동료가 아는" />
          <AxisLabel text="동료가 모르는" />
        </div>
      ...
      {topBlind && (
        <Card className="flex gap-3.5 items-start" style={{ padding: 18, marginTop: 18 }}>
          ...
          <p className="text-[14.5px] leading-relaxed">
            동료 {topBlind.cnt}명이 공통으로 <b style={{ color: QUAD.blind.color }}>{topBlind.ko}</b>({topBlind.en})을(를)
            골랐는데, 스스로는 나를 그렇게 보지 않았어요. 좋다·나쁘다를 떠나, 이 모습이 어떻게 느껴지세요? — 오늘 새로
            돌아볼 지점이에요.
          </p>
        </Card>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        {(["open", "blind", "hidden", "unknown"] as const).map((k) => {
          ...
          return (
            <div
              key={k}
              className="rounded-xl bg-panel border border-line p-4"
      ...
      <p className="text-faint text-xs mt-5 leading-relaxed">
        조하리의 창은 진단이 아니라 대화의 출발점이에요. 창을 함께 보며 "왜 이 단어가 여기 있을까"를 이야기하는 그
        과정에 가장 큰 가치가 있어요.
      </p>
    </div>
  );
